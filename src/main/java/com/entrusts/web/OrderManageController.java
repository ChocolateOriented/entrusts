...
public class OrderManageController {
	...
	public CommonResponse<Page<OrderView>> listHistoryOrder(OrderQuery orderQuery, Integer pageNum, Integer pageSize, HttpServletRequest request) {
		String userCode = request.getParameter("Account-Code");
		orderQuery.setUserCode(userCode);
		CommonResponse<Page<OrderView>> response = new CommonResponse<>();
		if (pageNum == null) {
			pageNum = 1;
		}
		if (pageSize == null) {
		...
		Page<OrderView> page = orderManageService.findHistoryOrder(orderQuery, pageNum, pageSize);
